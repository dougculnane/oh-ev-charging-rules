...
public class EVChargingRules extends JRule {
	...
	public void evcr_ExportPowerRule(JRuleItemEvent event) {
		...
		if (isTimeLocked(CHARGER_1_LOCK_NAME)) {
			logInfo(CHARGER_1_LOCK_NAME + ": locked so skipping export power change");
		...
		if (isTimeLocked(CHARGER_2_LOCK_NAME)) {
			logInfo(CHARGER_2_LOCK_NAME + ": locked so skipping export power change");
	...
	public void pollChargers(JRuleTimerEvent event) {
		...
		if (isTimeLocked(CHARGER_1_LOCK_NAME)) {
			logInfo(CHARGER_1_LOCK_NAME + ": locked so skipping polling");
		} else {
		...
		if (isTimeLocked(CHARGER_2_LOCK_NAME)) {
			logInfo(CHARGER_2_LOCK_NAME + ": locked so skipping polling");
		} else {
